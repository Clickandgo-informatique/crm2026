...
export async function createCalendarGrid(
    ...
    for (let i = 0; i < numberOfDays; i++) {
        ...
        console.log("column", i, formatDate(date));
        await createDayColumn(grid, date, events);
    }
}
function createTimeColumn(grid: HTMLElement): void {
    ...
    for (let hour = START_HOUR; hour < END_HOUR; hour += SLOT_DURATION / 60) {
...
async function createDayColumn(
    ...
    headerDayNumber.textContent = dayNumber.toString();
    ...
    await createPlanningEvents(dayColumn, date, events);
